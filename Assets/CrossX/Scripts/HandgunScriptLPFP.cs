...
public class HandgunScriptLPFP : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isGrabed)
        {
            ...
            {
                triggered = false;
                triggerBackedToTop = true;
            }

            if (triggerBackedToTop && triggered && device.TryGetFeatureValue(UnityEngine.XR.CommonUsages.trigger, out triggerValue) && !isReloading && !isInspecting && !isRunning && !outOfAmmo)
            {
                triggerBackedToTop = false;
                anim.Play("Fire", 0, 0f);
            ...
            if (device.TryGetFeatureValue(UnityEngine.XR.CommonUsages.primaryButton, out primaryButtonDown) && primaryButtonDown && !isReloading && !isInspecting)
            {
                //Reload
                Reload();
